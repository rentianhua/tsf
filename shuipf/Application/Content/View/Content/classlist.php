...
<div class="wrap J_check_wrap">
  <div class="nav">
    <ul class="cc">
      <li <if condition=" (!isset($_GET['status'])) AND (!isset($_GET['apply_state']))">class="current"</if>><a href="{:U('Content/classlist', array('catid'=>$catid)  )}">{$catname}列表（{$sum}条）</a></li>
      <li <if condition=" isset($_GET['status']) ">class="current"</if>><a href="{:U('Content/classlist', array('catid'=>$catid ,'search'=>1 ,'status'=>1)  )}">待审核（{$checkSum}条）</a></li>
      <if condition="($catid eq 6) OR ($catid eq 8)">
      <li <if condition=" isset($_GET['apply_state']) ">class="current"</if>><a href="{:U('Content/classlist', array('catid'=>$catid ,'search'=>1 ,'apply_state'=>1)  )}">申请中（{$applySum}条）</a></li>
	  </if>
    </ul>
  </div>
  <div class="mb10">
  <?php
$ct = array("3", "7", "33", "35", "38", "39", "40", "42", "43", "53", "54");
if (in_array($_GET['catid'], $ct)) { ?>
...
<?php }?> 		
  ...
  <if condition="$_GET['apply_state'] eq 1">
  	<input type="hidden" value="{$_GET['apply_state']}" name="apply_state">
  </if>
    ...
    <div class="search_type cc mb10">
      <div class="mb10"> 
        ...
        <select class="select_2" name="searchtype" style="width:70px;">
          ...
          <option value='1' <if condition=" $searchtype == '1' "> selected</if>>房源编号</option>
          <option value='2' <if condition=" $searchtype == '2' "> selected</if>>简介</option>
          <option value='3' <if condition=" $searchtype == '3' "> selected</if>>用户名</option>
          <option value='4' <if condition=" $searchtype == '4' "> selected</if>>ID</option>
        </select>
        关键字：
  ...
  <form class="J_ajaxForm" action="" method="post">
    <div class="table_list">
      <table width="100%">
        ...
        ?>
        <?php
        if($_GET['catid'] == 6||$_GET['catid'] == 3||$_GET['catid'] == 7||$_GET['catid'] == 8||$_GET['catid'] == 54){
          echo '<col width="140">';
        }
        ?>
        <?php
        if($_GET['catid'] == 6||$_GET['catid'] == 8){
          echo '<col width="140">';
        }
        ?>
        <col width="80">
        ...
        <volist name="data" id="vo">
          <tr>
            ...
            <td><if condition=" $vo['status'] eq 99 "><a href="{$vo.url}" target="_blank"><span style="" >
               <if condition="$vo['apply_state'] eq 1">
               <font color="#FF0000">[申请已售出]</font> - 
               </if>
               {$vo.title}</span></a>
                <else/>
            ...
            <?php
            if($_GET['catid'] == 53){
				$new = getnewName($vo['new_id']);
              echo '<td><a target="_blank" href="'.$new['url'].'">'.$new['title'].'</a></td>';
            }
            ?>
            <?php
            if($_GET['catid'] == 6||$_GET['catid'] == 3||$_GET['catid'] == 7||$_GET['catid'] == 8||$_GET['catid'] == 54){
              echo '<td>'.getareaName($vo['city']).' / '.getareaName($vo['area']).'</td>';
            }
            ?>
            <?php
            ...
            if($_GET['catid'] == 6||$_GET['catid'] == 8){
              if($vo['pub_type'] == 1){
				  echo '<td>自售</td>';
				}else if($vo['pub_type'] == 2){
					echo '<td>委托给经纪人</td>';
				}else if($vo['pub_type'] == 3){
					echo '<td>委托给平台</td>';
				}
            }
            ?>
            ...
            <td align="center"><if condition=" $vo['sysadd'] ">{$vo.username}
                <else />
                <font color="#FF0000">{$vo.username} | <if condition="$vo['modelid'] eq 35">普通用户<else />经纪人</if></font><img src="{$config_siteurl}statics/images/icon/contribute.png" title="会员投稿"></if></td>
    ...
    <div class="btn_wrap">
      <div class="btn_wrap_pd">
        ...
        <button class="btn J_ajax_submit_btn" type="submit" data-action="{:U('Content/public_check',array('catid'=>$catid,'apply_state'=>$_GET['apply_state']))}">审核</button>
