...
class QiugouController extends Base {
    ...
    public function add()
    {
      if (IS_POST) {
        ...
        if ($rs && $rs1 ) {
          $db->commit();
          //发布成功，跳转到我的求租          
          $this->success('添加成功', U('Member/User/qiugou'));
        } else {
          $db->rollback();
          $this->error('添加失败');
        }
      } else {
        $this->display("Qiugou:add");
      }
    }
	...
	public function del() {
		...
		$lock = M('userqiugou')->where($x)->getfield('lock');
		if($lock == 1){
			$this->success('删除失败！');
			exit;
		}
        $rs = M('userqiugou')-> where($x) -> delete();
		//插入一条记录到"history"表中
        $h['catid'] = 48;
        $h['status'] = 99;
