...
class ContentController extends AdminBase {
    ...
    public function classlist() {
		...
		$applySum = $model->where(array_merge($where, array('apply_state' => 1)))->count();
        $this->assign('sum', $sum)->assign('checkSum', $checkSum)->assign('applySum', $applySum);
        //搜索
        $search = I('get.search');
        if (!empty($search)) {
            ...
            if (!empty($keyword)) {
                ...
                $type_array = array('title', 'bianhao','description', 'username');
                if ($searchtype < 4) {
			...
			//申请中
			$apply_state = I('get.apply_state');
            if ($apply_state) {
                $where['apply_state'] = array("EQ", $apply_state);
            }
        }
        //信息总数
        if (empty($search)) {
		...
		foreach($data as $k=>$v){
			...
			$data[$k]['modelid'] = M('member')->where($aa)->getField('modelid');
		}
        $this->assign($catInfo)
    ...
    public function add() {
        if (IS_POST) {
            ...
            if ($category['type'] == 0) {
				...
				/*edd by wst start*/
			if($_POST['info']['catid']==3 || $_POST['info']['catid']==6 || $_POST['info']['catid']==8){
				//判断小区是否存在
			  $k['area'] = $_POST['info']['area'];
			  $k['title'] = $_POST['info']['xiaoquname'];
			  $r1 = M('xiaoqu')->where($k)->getfield('id');
			  $_POST['info']['xiaoqu']=$r1;
			  if (!$r1) {
				//插入小区
				$k['catid'] = 54;
				$k['username'] = 'admin';
				...
				$k['province'] = $_POST['info']['province'];
				$k['city'] = $_POST['info']['city'];
				$k['jingweidu'] = $_POST['info']['jingweidu'];
				
				$r2 = M('xiaoqu')->add($k);				
				$_POST['info']['xiaoqu']=$r2;
				if ($r2) {
				  //插入小区附表
				  $u['id'] = $r2;
				  $u['content'] = "";
		  ...
		  /*edd by wst end*/
                ...
                if ($status) {
					//新房和大宗交易需要添加房源编号
					if($catid==3){
						$kk['bianhao'] = "X".date("Ymd").($catid*10000000+$status);
						M('new')->where('id='.$status)->save($kk);
					}
					if($catid==7){
						$kk['bianhao'] = "D".date("Ymd").($catid*10000000+$status);
						M('dadazong')->where('id='.$status)->save($kk);
					}
    ...
    public function public_check() {
        if (IS_POST) { 
            ...
            foreach ($ids as $id) {
				if($_GET['apply_state'] == 1){
					if($this->catid == 6){
						M('ershou')->where('id='.$id)->save(array('zaishou'=>0,'apply_state'=>0));
					}
					if($this->catid == 8){
						M('chuzu')->where('id='.$id)->save(array('zaizu'=>0,'apply_state'=>0));
					}
				}
