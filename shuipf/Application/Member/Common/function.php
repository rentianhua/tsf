...
function getBT($fromid, $fromtable){
	$u['id'] = $fromid;
	$rs = M($fromtable) -> where($u) -> field('title,url') -> find();
	return $rs;
}
//根据id获取经纪人名称
function getjjrName($id){
	$rs = M('member') -> where('userid='.$id) -> getfield('realname');
	return $rs;
}
...
//根据id返回大宗交易名称
function getdzName($id){
	if($id){
  $name = M('dadazong') -> where('id='.$id) -> field('title,url') -> find();
  return $name;		
		}else{
	return "";
  }
...
//根据id返回优惠券使用说明
function getyhqDesc($id){
	if($id){
  $name = M('yhquan') -> where('id='.$id) -> getField('description');
  return $name;		
		}else{
	return "";
  }
