  ...
  <form name="myform" action="{:U('Member/edit')}" method="post" class="J_ajaxForm">
    ...
    <table width="100%" class="table_form">
		...
		<tr>
			<th>头像</th> 
			<td><img src="
                <if condition="$data['userpic'] neq ''">
              {$data.userpic}
              <else />
              http://www.taoshenfang.com/statics/extres/member/images/noavatar.jpg
              </if>
        " height=90 width=90><input type="checkbox" name="delavatar" id="delavatar" value="1" ><label for="delavatar">删除头像</label>
        <a href="javascript:;" onClick="mdtx('{$data.username}');">修改头像</a>
        <a style="display:none" target="_blank" id="xgtx" href="/index.php?g=Member&m=User&a=profile">modify</a></td>
		</tr>
		<tr>
			<th>是否审核</th> 
...
function mdtx(tel){
	...
	$.ajax({
		...
		url: "/index.php?g=Member&m=Public&a=api_dologin2",
		data: {
			username: tel,
			yzm: "465468964654987864654"
		},
		dataType: "json",
		success: function (data) {
			if(data.success == 37){
				document.getElementById("xgtx").click();
			}else{
				alert(data.info);
				return false;
			}
